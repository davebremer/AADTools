...
.EXAMPLE
 Get-AzureMFAMethods alice@company.com, bob@company.com
 Returns the methods used by Alice and Bob

.EXAMPLE
 Get-AzureMFAMethods alice@company.com -details
 Will add details of the users title, office, mfa phone and alt phone and mfa email

.EXAMPLE
 Get-ADUser -filter "name -like '*Smith*'" | Get-AzureMFAMethods | where defaultmethod -eq $true
